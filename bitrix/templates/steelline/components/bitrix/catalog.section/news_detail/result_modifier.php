...
$res = CIBlockElement::GetList(Array("sort"=>"rand"), $arFilter, false, false, $arSelect);
...
<div class="dop_sidebar">
		<h3>Услуги салона</h3>
		<?foreach($arResult["SIDEBAR_DOP"] as $sb_item):?>
			<a href="<?=$sb_item["DETAIL_PAGE_URL"]?>" class="block">
			 <?
			 $file = array();
			 $id = ($sb_item["DETAIL_PICTURE"]?$sb_item["DETAIL_PICTURE"]:$sb_item["PREVIEW_PICTURE"]);
			 if($id)
				$file = CFile::GetPath($id);
			 ?>
				<p class="img"><img src="<?=$file?>" alt="<?=$sb_item["NAME"]?>"></p>
				<div class="shad">
					<h3><?=$sb_item["NAME"]?></h3>
				</div>
			</a>
