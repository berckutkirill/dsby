            ...
            foreach ($arResult["ITEMS"] as $pos => $arItem):
                ...
                <div class="cart <?= implode(" ", $arItem["PROPERTIES"]["DESTINATION_ICON"]["VALUE_XML_ID"]); ?> clearfix">
                    <div class="left">
                        ...
                        <?php } ?>
                        <a href="<?= $arItem["DETAIL_PAGE_URL"] ?>"  onclick="onProductClick('<?= $arItem["ID"] ?>', '<?= $arItem["NAME"] ?>', '<?= $pos ?>', '<?= $arItem["DETAIL_PAGE_URL"] ?>');
                                return !ga.loaded;" class="img">
                        ...
                        <span class="price <?= $arItem["MIN_PRICE"]["DISCOUNT_VALUE"] == $arItem["MIN_PRICE"]["VALUE"] ? "blue_color" : ""; ?> js_price_gen">
                            <span class="new_rub"><?= $arItem['PROPERTIES']['IN_STOCK']['VALUE'] != "Y" ? "от " : ""; ?>
                                <span class="js_denomination_price"><?= toPrice($arItem["MIN_PRICE"]["DISCOUNT_VALUE"]) ?></span> руб.</span><br>
                    ...
                    <div class="right">
                        ...
                        <p class="img">
                            <?
                            if ($arItem["PROPERTIES"]["DOUBLE"]["VALUE"] == "Y" && $arItem["PROPERTIES"]["DOUBLE_PICTURE_2"]["VALUE"]) {
                                $dobor2 = CFile::GetPath($arItem["PROPERTIES"]["DOUBLE_PICTURE_2"]["VALUE"]);
                            } else {
                                $dobor2 = "";
                            }
                            ?>
                        ...
                        <a href="<?= $arItem["DETAIL_PAGE_URL"] ?>" onclick="onProductClick('<?= $arItem["ID"] ?>', '<?= $arItem["NAME"] ?>', '<?= $pos ?>', '<?= $arItem["DETAIL_PAGE_URL"] ?>');
                                return !ga.loaded;" class="enter">Узнать больше</a>
                    </div>
                    <div class="yarl"></div>
                </div>
