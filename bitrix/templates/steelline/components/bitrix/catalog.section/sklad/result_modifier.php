...
{
	if(is_array($_GET["so"])) {
		$filter["SKLAD_OPENER"] = array_map("strtolower", $_GET["so"]);
	} else {
		$filter["SKLAD_OPENER"] = trim(strtolower($_GET["so"]));
	}
...
{
	if(is_array($_GET["sw"])) {
		$filter["SKLAD_WIDTH"] = array_map("strtolower", $_GET["sw"]);
	} else {
		$filter["SKLAD_WIDTH"] = trim(strtolower($_GET["sw"]));
	}
}
...
{
  $id = $arItem["ID"];
	$name = $arItem["PROPERTIES"]["SKLAD_NAME"]["VALUE"]?$arItem["PROPERTIES"]["SKLAD_NAME"]["VALUE"]:$arItem["NAME"];
    foreach($arItem["PROPERTIES"]["SKLAD_SIZE_OPEN"]["VALUE"] as $k => $SKLAD_SIZE_OPEN)
    {
		preg_match("#([\d]*\s\w{2})\s(.*)$#u", $SKLAD_SIZE_OPEN, $matches);
		$SKLAD_OPENER = $matches["2"];
		$SKLAD_WIDTH = $matches["1"];
		
		if($filter["SKLAD_OPENER"]) {
			if(!in_array(trim(strtolower($SKLAD_OPENER)), $filter["SKLAD_OPENER"])) continue;
		}
		
		if($filter["SKLAD_WIDTH"]) {
			if(!in_array(trim(strtolower($SKLAD_WIDTH)), $filter["SKLAD_WIDTH"])) continue;
		}
		
		$XID = $arItem["PROPERTIES"]["SKLAD_SIZE_OPEN"]["VALUE_XML_ID"][$k];
		if($SKLAD_OPENER && $SKLAD_WIDTH) {
			$ITEMS[] = array("NAME" => $name, "ID" => $id."_".$XID, "SKLAD_OPEN" => $SKLAD_OPENER, "SKLAD_WIDTH" => $SKLAD_WIDTH, "PRICE" => $arItem["MIN_PRICE"]["VALUE"]);
		}
    }
