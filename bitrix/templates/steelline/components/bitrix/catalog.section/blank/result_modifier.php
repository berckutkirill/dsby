...
foreach($arResult["ITEMS"] as $arItem)
{
  ...
  $name = $arItem["PROPERTIES"]["SKLAD_NAME"]["VALUE"]?$arItem["PROPERTIES"]["SKLAD_NAME"]["VALUE"]:$arItem["NAME"];
	...
	foreach($arItem["PROPERTIES"]["SKLAD_SIZE_OPEN"]["VALUE"] as $k => $SKLAD_SIZE_OPEN)
	{
		...
		if(!$CART[$id."_".$XID]) continue;
		if($arItem["PROPERTIES"]["SKLADSKAYA"]["VALUE"] == "Y") $status = "склад"; else $status = "конструктор";
		$ITEMS[] = array("NAME" => $name, "ID" => $id."_".$XID, "PRICE" => intVal($arItem["MIN_PRICE"]["VALUE"]), "STATUS" => $status);
