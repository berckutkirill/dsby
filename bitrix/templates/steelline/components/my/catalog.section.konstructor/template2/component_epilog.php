	...
	for(var i in models){
		...
		if(models[i]["KOL"] == colid)
		{
			...
			if(can) { 
				...
				inHtml += "<li data-text='"+models[i]["VALUE"]+"' class='"+classVal+"' data-price='"+models[i]["BASE_PRICE"]+"' data-value='"+models[i]["VALUE_ID"]+"'>"+models[i]["VALUE"]+"<span>"+models[i]["BASE_PRICE"]+"</span></li>";
			}
		}
	}
...
function hideInv() {
	if($("#collection").attr("data-value") == "72")
	{
		...
		if($(".check[data-input='HOME']").attr("data-value") == "63"){
			...
			$(".texture_list li[data-choise='1']").addClass("disabled");
			$(".texture_list li[data-choise='2']").removeClass("disabled");
...
function reloadForm(){
			for(var i in objChanger){
				...
				if(exc == "SHIR X VIS") {
					...
					value = TOL+" "+SEC+" "+COLOR;
				} else if(exc == "GERK + NAKL") {
					var GERK = $(".excel[data-excel='GERK']").attr("data-excel-val");
					var NAKL = $(".excel[data-excel='NAKL']").attr("data-excel-val");

					value = GERK+" "+NAKL;

				} else {
					value = $(".excel[data-excel='"+exc+"']").attr("data-excel-val")
				}
				var filt = i;

				if(value && $(".exel_table input."+i).length) {
	...
	mini_slider('.colors_door.three', 1, 120, 5, 1,250);
		...
		$('[data-radio]').on('click', function() {
			...
			if($(this).closest(".switcher_set").length) {
				...
				reloadForm();
			} else if($(this).attr("data-excel")){
				reloadForm();
			} else if($(this).closest(".furn_type").length){
		...
		$(".end_constr button").unbind();
		$(".end_constr button").on('click', function() {
			addZayavka();
		});
		reloadForm();
		checkAllPrice();
