...
if($IDS = $arResult["PROPERTIES"]["ELEMENTS_IN"]["VALUE"])
{
	...
	if($PROPERTIES["WIDE_CART"]["VALUE"] == "Y")
	{
		$arFields["WIDE_CART"] = 1;
	} else {
		$arFields["WIDE_CART"] = 0;
	}
	 foreach($PROPERTIES as $code => $val)
	 {
		if(strpos($code,"CHARACKTER_") === 0)
		{
			if($val["VALUE"])
			{
				...
				{
					$arFields["PROPERTIES"][$code]["TEXT"] = $val["VALUE"];
					$arFields["PROPERTIES"][$code]["NAME"] = $val["NAME"];
				}
			}
