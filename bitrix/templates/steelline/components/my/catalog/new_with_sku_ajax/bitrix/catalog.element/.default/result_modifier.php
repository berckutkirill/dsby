...
usort($arResult["OFFERS"], 'minP');
$MF = in_array("flat", $arResult["PROPERTIES"]["APPOINTMENT"]["VALUE_XML_ID"]) ? "для квартиры" : in_array("home", $arResult["PROPERTIES"]["APPOINTMENT"]["VALUE_XML_ID"]) ? "для дома" : "";
...
$arResult["TABS_HEAD"] = ["DECOR", "CONST", "LOCK", "EXTRA"];
$arResult["THEAD"] = [
    "DECOR" => ["THEAD_DECOR_NAME", "THEAD_DECOR_OUT", "THEAD_DECOR_IN"],
    "CONST" => ["THEAD_CONST_NAME", "THEAD_CONST_VALUE"],
    "LOCK" => ["THEAD_LOCK_NAME", "THEAD_LOCK_VALUE"],
    "EXTRA" => ["THEAD_EXTRA_NAME", "THEAD_EXTRA_VALUE"]
];

$arResult["FROM_OFFERS"] = ["DECOR", "LOCK"];

//print_r($arResult);
if ($arResult["PROPERTIES"]["IN_STOCK"]["VALUE"] == "Y" || $arResult["PROPERTIES"]["COLLECTION"]["VALUE"] == COLLECTION_SEVER || $arResult["IBLOCK_SECTION_ID"] == 57) {
    $MAIN_CHARS = ["CHARAKTER_CONST_SEALING_CONTOURA_MAIN", "CHARAKTER_CONST_HEIGHT_MAIN", "CHARAKTER_CONST_WIDTH_MAIN"];
} else {
    $MAIN_CHARS = ["CHARAKTER_CONST_THICKNESS_METAL_MAIN", "CHARAKTER_CONST_SEALING_CONTOURA_MAIN", "CHARAKTER_CONST_CATEGORY_OF_DURABILITY_MAIN"];
}
$arResult["PROPS"] = makeProps($arResult["PROPERTIES"], $MAIN_CHARS);
if ($arResult["PROPERTIES"]["LOCK_SYSTEM"]["VALUE"]) {
    $arSelect = ["NAME", "ID", "IBLOCK_ID"];
    $arFilter = ["ID" => $arResult["PROPERTIES"]["LOCK_SYSTEM"]["VALUE"]];
    ...
    if ($arFields) {
        $arResult["PROPS"] = array_merge($arResult["PROPS"], makeProps($arFields["PROPERTIES"]), $MAIN_CHARS);
    }
}


$LOCKS = $arResult["PROPS"]["LOCK"];
unset($arResult["PROPS"]["LOCK"]);
foreach ($arResult["OFFERS"] as &$offer) {
    if ($offer["DETAIL_PICTURE"]["SRC"] && $offer["PROPERTIES"]["DECOR_PHOTO_IN"]["VALUE"]) {
        ...
        if (!$arResult["DETAIL_PICTURE_OUT"]) {
            $arResult["DETAIL_PICTURE"] = $offer["DETAIL_PICTURE"];
            $arResult["DETAIL_PICTURE_OUT"] = ["SRC" => CFile::GetPath($offer["PROPERTIES"]["DECOR_PHOTO_IN"]["VALUE"]), "ALT" => $offer["NAME"]];
        }
    }
    $offer["TOOLTIP"]["ARTICLE"] = $offer["PROPERTIES"]["DECOR_ARTICLE"]["VALUE"];

    $offer["TOOLTIP"]["OUTSIDE"] = $offer["PROPERTIES"]["CHARAKTER_DECOR_TECH_OUT"]["VALUE"] ? $offer["PROPERTIES"]["CHARAKTER_DECOR_TECH_OUT"]["VALUE"] : $offer["PROPERTIES"]["CHARAKTER_DECOR_COVER_OUT"]["VALUE"];
    $offer["TOOLTIP"]["OUTSIDE"] .= ", " . $offer["PROPERTIES"]["CHARAKTER_DECOR_COLOR_OUT"]["VALUE"];
    $offer["TOOLTIP"]["INSIDE"] = $offer["PROPERTIES"]["CHARAKTER_DECOR_TECH_IN"]["VALUE"] ? $offer["PROPERTIES"]["CHARAKTER_DECOR_TECH_IN"]["VALUE"] : $offer["PROPERTIES"]["CHARAKTER_DECOR_COVER_IN"]["VALUE"];
    $offer["TOOLTIP"]["INSIDE"] .= ", " . $offer["PROPERTIES"]["CHARAKTER_DECOR_COLOR_IN"]["VALUE"];
    $props = makeProps($offer["PROPERTIES"], $MAIN_CHARS);
    $arResult["PROPS"]["DECOR"][$offer["ID"]] = $props["DECOR"];
    $props["LOCK"] = $props["LOCK"] ? $props["LOCK"] : [];
    $arResult["PROPS"]["LOCK"][$offer["ID"]] = array_merge($LOCKS, $props["LOCK"]);
}
$SIMILAR_ELEMENTS = [];
CModule::IncludeModule('catalog');
$res = CIBlockPriceTools::GetCatalogPrices(1, array('BASE'));

if ($arResult["PROPERTIES"]["COLLECTION"]["VALUE"]) {
    $arSelect = ["ID", "IBLOCK_ID", "NAME", "CATALOG_GROUP_1", "PREVIEW_PICTURE", "DETAIL_PAGE_URL"];
    $double_flag = ($arResult["PROPERTIES"]["DOUBLE"]["VALUE"] == "Y") ? "Y" : false;
    $arFilter = ["IBLOCK_ID" => $arResult["IBLOCK_ID"], "ACTIVE" => "Y", "PROPERTY_COLLECTION_VALUE" => $arResult["PROPERTIES"]["COLLECTION"]["VALUE"], "!ID" => $arResult["ID"], "PROPERTY_DOUBLE_VALUE" => $double_flag];
    $res = CIBlockElement::GetList(["sort" => "rand"], $arFilter, false, ["nTopCount" => 12], $arSelect);
    while ($ob = $res->GetNextElement()) {
        $arFields = $ob->getFields();
        $arFields["PROPERTIES"] = $ob->getProperties();
        $MIN_OFFER = getOffersWithMinPrice($arFields["ID"], false,true);
        $arFields["MIN_PRICE"] = $MIN_OFFER["MIN_PRICE"];
        $arFields["PICTURE_1"] = $MIN_OFFER["DETAIL_PICTURE"];
        $arFields["PICTURE_2"] = $MIN_OFFER["DETAIL_PICTURE_2"];
        $arResult["SIMILAR_ELEMENTS"][] = $arFields;
    }
}

if ($arResult["PROPERTIES"]["SAMPLE"]["VALUE"]) {

    $db_props = CIBlockElement::GetProperty(IBLOCK_MAP, $arResult["PROPERTIES"]["SAMPLE"]["VALUE"], array("sort" => "asc"), Array("CODE" => "ADRESS"));
    if ($ar_props = $db_props->Fetch()) {
        $address = $ar_props["VALUE"];
    }
    $arResult["MAP"] = ["ID" => $arResult["PROPERTIES"]["SAMPLE"]["VALUE"], "ADDRESS" => $address];
}
