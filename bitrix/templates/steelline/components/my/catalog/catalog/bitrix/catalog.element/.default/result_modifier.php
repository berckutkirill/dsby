...
if($arResult["PROPERTIES"]["NABOR_FURNITUR"]["VALUE"]) {
	$db_props = CIBlockElement::GetProperty(18, $arResult["PROPERTIES"]["NABOR_FURNITUR"]["VALUE"], array("sort" => "asc"), Array("CODE"=>"ELEMENTS_IN"));
	...
	if($IDS) {
		$furnFilter = array("ID" => $IDS);
		
		$arSelect = Array("ID", "IBLOCK_ID", "NAME", "PREVIEW_TEXT","DETAIL_PICTURE","PROPERTY_TOOLTIP", "PROPERTY_NAME");
		
		$res = CIBlockElement::GetList(array("sort" => "desc"), $furnFilter, false, Array("nPageSize"=>50), $arSelect);
		while($ob = $res->GetNextElement()){ 
			$arFields = $ob->GetFields();  
			$code = $arFields["ID"];

			$FILE_TTH[$code]["FILE"] = CFile::GetPath($arFields["DETAIL_PICTURE"]);
			$FILE_TTH[$code]["TEXT"] = $arFields["PREVIEW_TEXT"];
			$FILE_TTH[$code]["TOOLTIP"] = $arFields["PROPERTY_TOOLTIP_VALUE"]["TEXT"];
			
			$FILE_TTH[$code]["NAME"] = $arFields["PROPERTY_NAME_VALUE"];
		}
	}
	$arResult["FURN_FILE_TTH"] = $FILE_TTH;
	
}

if($arResult["PROPERTIES"]["COLOR_NEW"]["VALUE"]) {
	$arResult["PROPERTIES"]["CHARAKTER_COLOR"]["VALUE"] .= ', '.$arResult["PROPERTIES"]["COLOR_NEW"]["VALUE"];
}
if($arResult["PROPERTIES"]["CHARAKTER_TINER"]["VALUE"]) {
	$arResult["PROPERTIES"]["CHARAKTER_TINER"]["VALUE"] .= ', '.$arResult["PROPERTIES"]["UPLOTNITEL"]["VALUE"];
}
...
$Settings = array("OTDELKA_SNARUZHI" => 
				...
				"NALICHNIK" => 
				array("NAME" => "Наличник", "TOOLTIP" => "TOOLTIP", "CODES" => array("OTDELKA", "POKRYTIE", "COLOR", "PATINA", "OTDELKA_VNUTRENIY")));

foreach($Settings as $tth_code => $tth_values_codes) {
	$NAME = $tth_values_codes["NAME"];
	$TOOLTIP = $arResult["PROPERTIES"][$tth_code."_".$tth_values_codes["TOOLTIP"]]["VALUE"];
	$TEXT = "";
	foreach($tth_values_codes["CODES"] as $key =>  $code) {
		$value = $arResult["PROPERTIES"][$tth_code."_".$code]["VALUE"];
		if($value) {
			if($TEXT) {
				if($tth_code == "NALICHNIK" && $code == "OTDELKA_VNUTRENIY") {
					$TEXT .= ";";
				} else {
					$TEXT .= ",";
				}
			...
			$TEXT .= " ".$sostav[$tth_code][$code] = $value;
		}
	}
	if($TEXT)
...
$arResult["SOSTAV_TTH"] = $sostav;

if($arResult['PROPERTIES']['COLOR_DESIGIONS']['VALUE']) {
	function getEnum($ID) {
	...
	$hblock = GetHBlock(15, ["UF_XML_ID" => $arResult['PROPERTIES']['COLOR_DESIGIONS']['VALUE']]);

	foreach($hblock as $eq)
	{

		$result[] = [
			'PIC_1' => CFile::GetPath($eq['UF_PIC_1']),
			'PIC_2' => CFile::GetPath($eq['UF_PIC_2']),
			'COLOR' => getEnum($eq['UF_COLOR'])
		];
	}
	$arResult['COLOR_DESIGIONS'] = $result;
}
if($arResult["PROPERTIES"]["SOME_NEW_MODEL"]["VALUE"]) {
		$filter = array( "ACTIVE" => "Y","ID" => $arResult["PROPERTIES"]["SOME_NEW_MODEL"]["VALUE"]);
		$arSelect = Array("ID", "ACTIVE", "GLOBAL_ACTIVE", "IBLOCK_ID", "NAME", "DETAIL_PAGE_URL");

		$res = CIBlockElement::GetList(array("sort" => "desc"), $filter, false, Array("nTopCount"=>1), $arSelect);
		while($ob = $res->GetNextElement()){ 
			$arFields = $ob->GetFields();  
			$arProps = $ob->GetProperties();

			$MIN_PRICE = getOffersWithMinPrice($arFields["ID"]);
			$arFields["MIN_PRICE"] = $MIN_PRICE["MIN_PRICE"];
			$arFields["PREVIEW_PICTURE"]["SRC"] = $MIN_PRICE["DETAIL_PICTURE"];
			$arFields["PREVIEW_PICTURE2"]["SRC"] = $MIN_PRICE["DETAIL_PICTURE_2"];
			
			$arFields["PROPERTIES"] = ["DESTINATION_ICON" => $arProps["DESTINATION_ICON"]];
			$arResult["SOME_NEW_MODEL"] = $arFields;
		}
}
