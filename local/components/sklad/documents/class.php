...
class SkladDocumentComponent extends CBitrixComponent {
    ...
    private function getDocuments() {
        ...
        while ($arRes = $dbResultList->NavNext()) {
            ...
            $arRes['DATE_DOCUMENT_TIME'] = $date->format('H:i');
        ...
        if ($requestData['filter']) {
            ...
            if ($filterArr['id']) {
                ...
                if ($filterArr['date'][0] == $filterArr['date'][1]) {
                    $data = new Bitrix\Main\Type\Date($filterArr['date'][0], 'd.m.y H:i:s');
                    $data2 = new Bitrix\Main\Type\Date($filterArr['date'][0], 'd.m.y H:i:s');
                    $data2->add('1D');
                } else {
                    $dataF = new Bitrix\Main\Type\Date($filterArr['date'][0], 'd.m.y H:i:s');
                    $dataS = new Bitrix\Main\Type\Date($filterArr['date'][1], 'd.m.y H:i:s');
                    $dataS->add('1D');
