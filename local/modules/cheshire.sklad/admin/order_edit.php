...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && $CHESHIRE_SKLAD_RIGHT == "W" && check_bitrix_sessid()) {
    ...
    if ($request->getPost("DATE_UPDATE") && check_bitrix_sessid()) {
        ...
        $arFields = [
            ...
            "DATE_UPDATE" => new Bitrix\Main\Type\DateTime($request->getPost("DATE_UPDATE")),
    ...
    if (!$res) {
        ...
        if (empty($request->getPost('apply')))
            LocalRedirect('/bitrix/admin/cheshire.sklad_orders.php?lang=' . LANGUAGE_ID);
...
$aContext = array(
    array(
        ...
        "LINK" => "cheshire.sklad_orders.php?lang=" . LANGUAGE_ID,
    ...
    if ($CHESHIRE_SKLAD_RIGHT == "W") {
        $aContext[] = array(
            ...
            "ONCLICK" => "javascript:if(confirm('" . getMessageJS("CONFIRM_DEL_MESSAGE") . "'))window.location='cheshire.sklad_orders.php?action=delete&ID[]=" . CUtil::JSEscape($ID) . "&lang=" . LANGUAGE_ID . "&" . bitrix_sessid_get() . "';",
        );
    }
...
<form method="post" action="<? $APPLICATION->GetCurPage() ?>">
    ...
    <? } ?>
    <tr class="adm-detail-required-field">
        <td width="40%"><? echo Loc::getMessage("CHESHIRE_SKLAD_USER_ID"); ?>:</td>
        <td width="60%">
            <input type="text" name="USER_ID" value="<? echo htmlspecialcharsbx($order['USER_ID']); ?>">
        </td>
    </tr>
    <tr class="adm-detail-required-field">
        <td width="40%"><? echo Loc::getMessage("CHESHIRE_SKLAD_FROM_ID"); ?>:</td>
        <td width="60%">
            <input type="text" name="FROM_ID" value="<? echo htmlspecialcharsbx($order['FROM_ID']); ?>">
        </td>
    </tr>
    <tr class="adm-detail-required-field">
        <td width="40%"><? echo Loc::getMessage("CHESHIRE_SKLAD_TOTAL"); ?>:</td>
        <td width="60%">
            <input type="text" name="TOTAL" value="<? echo htmlspecialcharsbx($order['TOTAL']); ?>">
        </td>
    </tr>
    <tr>
        <td width="40%"><? echo Loc::getMessage("CHESHIRE_SKLAD_DATE_UPDATE"); ?>:</td>
        <td width="60%">
            <input type="text" name="DATE_UPDATE" value="<? echo htmlspecialcharsbx($order['DATE_UPDATE']); ?>">
        </td>
    </tr>
   <tr>
        <td width="40%"><? echo Loc::getMessage("CHESHIRE_SKLAD_STATUS"); ?>:</td>
        <td width="60%">
            <input type="text" name="STATUS" value="<? echo htmlspecialcharsbx($order['STATUS']); ?>">
        </td>
    </tr>
    <?
    $tabControl->EndTab();
